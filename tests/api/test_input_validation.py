...
def test_passport_invalid_run_id_returns_400() -> None:
    client = TestClient(app)
    response = client.get("/passports/bad%5Crun")
    assert response.status_code == 400
    assert "invalid run_id" in response.json()["detail"]


def test_create_run_rejects_external_profile_path(tmp_path: Path) -> None:
    ext_profile = tmp_path / "external_profile.json"
    ext_profile.write_text('{"model":"x"}', encoding="utf-8")

    client = TestClient(app)
    response = client.post("/runs", json={"profile": str(ext_profile)})
    assert response.status_code == 400
